 ...
 *
 * IP地址划分:
 * A类地址：1.0.0.1～126.255.255.254 ，A类地址分配给规模特别大的网络使用
 * B类地址：128.0.0.1～191.255.255.254 ，B类地址分配给一般的中型网络。
 * C类地址：192.168.0.0～192.168.255.255 ，C类地址分配给小型网络，如一般的局域网和校园网，它可连接的主机数量是最少的，采用把所属的用户分为若干的网段进行管理。
 * D类地址：224.0.0.1～239.255.255.254 ，D类地址称为广播地址，供特殊协议向选定的节点发送信息时用。
 * E类地址：240.0.0.1～255.255.255.254 ，E类地址保留给将来使用。
 *
 * 如何判断两个IP地址是否是同一个网段中:
...
public class IpV4Util {
     ...
     */
    public static InetAddress[] getAllLocalInetAddress() throws UnknownHostException {
        return InetAddress.getAllByName(InetAddress.getLocalHost().getHostAddress());
    }
    /**
     * 获取所有本地ip
     * @return
     * @throws UnknownHostException
     */
    public static String[] getAllLocalIp() throws UnknownHostException {
        InetAddress[] iaArr = getAllLocalInetAddress();
        String[] ips = new String[iaArr.length];
        for (int i = 0; i < iaArr.length; i++) {
            ips[i] = iaArr[i].getHostName();
        }
        return ips;
    }

    public static String getLocalCTypeIpV4() throws UnknownHostException {
        InetAddress[] iaArr = getAllLocalInetAddress();
        for (int i = 0; i < iaArr.length; i++) {
            String ip = iaArr[i].getHostName();
            if (ipV4Validate(ip) && checkIpV4Type(ip) == IP_C_TYPE){
                return ip;
            }
        }
        throw new UnknownHostException("未找到C类ip");
    }
